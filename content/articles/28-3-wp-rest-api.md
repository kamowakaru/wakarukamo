...
**WordPress REST APIで投稿数・公開記事を確認する方法**には、対象を確認してから手順を一つずつ進め、最後に結果まで確認します。
...
```text
https://example.com/wp-json/wp/v2/posts?status=publish&per_page=1
```
レスポンスヘッダーの `X-WP-Total` で該当投稿の総数、`X-WP-TotalPages` で総ページ数を確認できます。本文データを全部取得して数えるより軽量です。
...
**WordPress REST APIで投稿数・公開記事を確認する方法**では、用語だけでなく実際に何を確認・操作すればよいかまで押さえることが大切です。
