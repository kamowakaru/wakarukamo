...
**WordPressでサイトマップを作る方法**には、対象を確認してから手順を一つずつ進め、最後に結果まで確認します。
...
## WordPressでサイトマップを用意する流れ

1. まず `https://自分のドメイン/wp-sitemap.xml` を開き、WordPress標準のXMLサイトマップが利用できるか確認します。
2. SEOプラグインで別のサイトマップを生成している場合は、そのURLと設定を確認します。
3. サイトマップURLをブラウザで開き、エラーにならないことを確認します。
4. Google Search Consoleの「サイトマップ」からURLを送信します。
5. 送信後はステータスを確認します。送信しただけで検索結果への掲載が保証されるわけではありません。
...
この記事のテーマでは、**SEOプラグイン** が選択肢になる場合があります。ただし、アフィリエイト候補だからという理由で選ぶのではなく、無料の方法も含めて目的に合うかを比較することが大切です。
...
**WordPressでサイトマップを作る方法**では、用語だけでなく実際に何を確認・操作すればよいかまで押さえることが大切です。
